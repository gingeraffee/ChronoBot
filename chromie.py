...
# Throttle noisy permission logs/DMs (background loop can run every minute)
LOG_THROTTLE_SECONDS = 60 * 30  # 30 minutes
_last_log: dict[tuple[int, str], float] = {}


def log_throttled(guild_id: int, code: str, message: str):
    now = datetime.now(tz=DEFAULT_TZ).timestamp()
    key = (guild_id, code)
...
def _bot_member_for_channel(channel: discord.TextChannel) -> discord.Member | None:
    guild = channel.guild
    if guild is None or guild.me is None:
        # guild.me can be None if member cache is cold; try get_member
        if guild is None:
            return None
        if bot.user is None:
            return None
...
def missing_channel_perms(channel: discord.TextChannel) -> list[str]:
    """Return a list of missing permissions ChronoBot needs in the event channel."""
    me = _bot_member_for_channel(channel)
    if me is None:
        return ["View Channel", "Send Messages", "Embed Links", "Manage Messages", "Read Message History"]
    ...
    # Needed to pin/unpin (and sometimes to manage pins generally)
    if not perms.manage_messages:
        missing.append("Manage Messages")
    # Strongly recommended for fetch_message / pin lookups
...
async def notify_owner_missing_perms(
    ...
    """Best-effort DM to server owner (throttled)."""
    if not missing:
        return
    guild_id = guild.id
    log_throttled(
        guild_id,
        code,
        f"[Guild {guild_id}] Missing permissions in #{channel.name}: {', '.join(missing)}",
    )

    # Throttle DMs too (same code key)
    now = datetime.now(tz=DEFAULT_TZ).timestamp()
    key = (guild_id, f"dm:{code}")
    last = _last_log.get(key, 0.0)
    if now - last < LOG_THROTTLE_SECONDS:
        return
    _last_log[key] = now
    ...
    try:
        ...
        await owner.send(
            ...
            + "\n\nFix: Update the bot's channel permissions (or role) and try again."
        )
    except Exception:
        # DMs can be disabled; ignore
        pass


def build_plaintext_for_guild(guild_state: dict) -> str:
    """Fallback text countdown if Embed Links permission is missing."""
    sort_events(guild_state)
    events = guild_state.get("events", [])
    if not events:
        return "No events set yet. Add one with /addevent."

    lines: list[str] = ["📅 **Server Events**"]
    any_upcoming = False
    for ev in events[:15]:
        dt = datetime.fromtimestamp(ev["timestamp"], tz=DEFAULT_TZ)
        desc, _, passed = compute_time_left(dt)
        owner_id = ev.get("owner_id")
        owner_tag = f" (owner: <@{int(owner_id)}>)" if owner_id else ""
        if passed:
            lines.append(f"• **{ev['name']}** — started/passed{owner_tag}")
        else:
            any_upcoming = True
            lines.append(f"• **{ev['name']}** — {desc} remaining{owner_tag}")

    if not any_upcoming:
        lines.append("All listed events have already started or passed.")
...
async def rebuild_pinned_message(guild_id: int, channel: discord.TextChannel, guild_state: dict):
    ...
    guild = channel.guild
    missing = missing_channel_perms(channel)
    if guild is not None and missing:
    ...
    try:
        # If Embed Links is missing, fall back to plain text so the loop doesn't crash.
        if "Embed Links" in missing:
            msg = await channel.send(content=build_plaintext_for_guild(guild_state))
        else:
            msg = await channel.send(embed=embed)
    except discord.Forbidden:
        log_throttled(
            guild_id,
            "countdown_send_forbidden",
            f"[Guild {guild_id}] Forbidden: can't send countdown message in #{channel.name}.",
        )
        return None
    ...
    try:
        ...
        log_throttled(
            guild_id,
            "pin_forbidden",
            f"[Guild {guild_id}] Missing permission to pin messages in #{channel.name}.",
        )
        ...
        log_throttled(
            guild_id,
            "pin_http",
            f"[Guild {guild_id}] Failed to pin message in #{channel.name}: {e}",
        )
...
async def get_or_create_pinned_message(guild_id: int, channel: discord.TextChannel):
    ...
    if pinned_id:
        try:
            ...
            # Message was deleted/unpinned; rebuild
            guild_state["pinned_message_id"] = None
            save_state()
        except discord.Forbidden:
            # Can't read/fetch in this channel
            missing = missing_channel_perms(channel)
            log_throttled(
                guild_id,
                "fetch_forbidden",
                f"[Guild {guild_id}] Forbidden: can't fetch pinned message in #{channel.name}.",
            )
            if channel.guild is not None:
                await notify_owner_missing_perms(
                    channel.guild,
                    channel,
                    missing,
                    action="read the pinned countdown message",
                    code="fetch_forbidden",
                )
            return None

    missing = missing_channel_perms(channel)
    if channel.guild is not None and missing:
        await notify_owner_missing_perms(
            channel.guild,
            channel,
            missing,
            action="create the pinned countdown message",
            code="create_pinned",
        )
    ...
    try:
        ...
        log_throttled(
            guild_id,
            "send_forbidden",
            f"[Guild {guild_id}] Forbidden: can't send pinned countdown message in #{channel.name}.",
        )
        return None
...
async def update_countdowns():
    ...
    for gid_str, guild_state in list(guilds.items()):
        ...
        # Update pinned message (embed if possible, plain text fallback if not)
        try:
            pinned = await get_or_create_pinned_message(guild_id, channel)
            if pinned is None:
                # Missing perms (send/view/etc.) – can't do anything in this channel
                continue
            ...
            try:
                if "Embed Links" in missing:
                    await pinned.edit(content=build_plaintext_for_guild(guild_state), embed=None)
                else:
                    await pinned.edit(content=None, embed=embed)
            except discord.Forbidden:
                log_throttled(
                    guild_id,
                    "edit_forbidden",
                    f"[Guild {guild_id}] Forbidden: can't edit countdown message in #{channel.name}.",
                )
                if channel.guild is not None:
                    await notify_owner_missing_perms(
                        channel.guild,
                        channel,
                        missing_channel_perms(channel),
                        action="edit the countdown message",
                        code="edit_forbidden",
                    )
                continue
            except discord.HTTPException as e:
                log_throttled(
                    guild_id,
                    "edit_http",
                    f"[Guild {guild_id}] Failed to edit countdown message in #{channel.name}: {e}",
                )
        except Exception as e:
            # Never let one guild crash the loop.
            log_throttled(guild_id, "loop_error", f"[Guild {guild_id}] update_countdowns error: {e}")
            continue
        ...
        for ev in guild_state.get("events", []):
            ...
            if days_left in milestones and days_left not in announced:
                ...
                try:
                    ...
                    log_throttled(
                        guild_id,
                        "milestone_send_forbidden",
                        f"[Guild {guild_id}] Forbidden: can't send milestone in #{channel.name}.",
                    )
                    if channel.guild is not None:
                        await notify_owner_missing_perms(
                            channel.guild,
                    ...
                    log_throttled(
                        guild_id,
                        "milestone_send_http",
                        f"[Guild {guild_id}] Failed to send milestone in #{channel.name}: {e}",
                    )
                    continue
